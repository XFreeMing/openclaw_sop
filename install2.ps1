...
<# One-click install, no parameters required #>
...
Write-Host "         One-Click Install" -ForegroundColor $AccentColor
Write-Host "  ======================================" -ForegroundColor $AccentColor
Write-Host ""
...
if ($DryRun) {
    ...
    # Configure git to use HTTPS instead of SSH (fixes libsignal dependency)
    # Some dependencies use git+ssh:// URLs which require SSH keys
...
if ($DryRun) {
    ...
    $onboardArgs = @(
        ...
        "--accept-risk",
        "--mode", "local",
        "--auth-choice", "skip",           # Skip auth - qwen-portal requires interactive OAuth
        "--gateway-port", "18789",
...
# ========== Step 2: Copy qwen-portal-auth extension to global extensions ==========
Write-Host ""
Write-Host "[*] Installing qwen-portal-auth extension..." -ForegroundColor $InfoColor

# openclaw-cn uses ~/.openclaw as state dir (see resolveConfigDir in utils.ts)
$openclawDir = "$env:USERPROFILE\.openclaw"
$extensionsDir = "$openclawDir\extensions"
$pluginDir = "$extensionsDir\qwen-portal-auth"

# Find the npm global packages directory and the extension source
$npmGlobalDir = $null
try {
    $result = npm root -g 2>$null
    if ($result) { $npmGlobalDir = $result.Trim() }
} catch {}

if ($npmGlobalDir -and (Test-Path "$npmGlobalDir\openclaw-cn\extensions\qwen-portal-auth")) {
    $extensionSource = "$npmGlobalDir\openclaw-cn\extensions\qwen-portal-auth"
    ...
    if (-not $DryRun) {
        ...
        # Copy extension if not already present or outdated
        if (Test-Path $pluginDir) {
            Remove-Item -Path $pluginDir -Recurse -Force
        }
        Copy-Item -Path $extensionSource -Destination $pluginDir -Recurse -Force
        
        Write-Host "[OK] Installed qwen-portal-auth extension" -ForegroundColor $SuccessColor
    } else {
        Write-Host "  [dry-run] Copy $extensionSource to $pluginDir" -ForegroundColor $MutedColor
    }
} else {
    Write-Host "[!] Could not find qwen-portal-auth extension in npm package" -ForegroundColor $WarnColor
    Write-Host "    Expected at: $npmGlobalDir\openclaw-cn\extensions\qwen-portal-auth" -ForegroundColor $MutedColor
}

# ========== Step 3: Patch JSON config files with qwen-portal settings ==========
Write-Host ""
Write-Host "[*] Patching configuration for Qwen Portal..." -ForegroundColor $InfoColor

# Update openclaw.json with models config (config is at ~/.openclaw/openclaw.json)
$openclawJson = "$openclawDir\openclaw.json"
if (Test-Path $openclawJson) {
    if (-not $DryRun) {
        try {
            $config = Get-Content -Path $openclawJson -Raw -Encoding UTF8 | ConvertFrom-Json
            ...
            # Enable qwen-portal-auth plugin
            ...
            # Explicitly set enabled: true for the plugin
            $pluginEntry = [PSCustomObject]@{ enabled = $true }
            $config.plugins.entries | Add-Member -NotePropertyName "qwen-portal-auth" -NotePropertyValue $pluginEntry -Force
            
            # If there's an allowlist, ensure qwen-portal-auth is in it
            if ($config.plugins.allow -and $config.plugins.allow -is [array]) {
                if ($config.plugins.allow -notcontains "qwen-portal-auth") {
                    $config.plugins.allow += "qwen-portal-auth"
                }
            }
            
            # Write back
            $config | ConvertTo-Json -Depth 20 | Out-File -FilePath $openclawJson -Encoding UTF8
            Write-Host "[OK] Updated openclaw.json with Qwen Portal config" -ForegroundColor $SuccessColor
        } catch {
            Write-Host "[!] Failed to patch openclaw.json: $_" -ForegroundColor $WarnColor
        }
    }
} else {
    Write-Host "[!] openclaw.json not found, skip patching" -ForegroundColor $WarnColor
}

# Update auth-profiles.json
$authProfilesJson = "$openclawDir\auth-profiles.json"
